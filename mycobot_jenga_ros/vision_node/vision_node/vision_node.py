...
class VisionNode(Node):
    def __init__(self):
        ...
        self.color_sub = self.create_subscription(
            Image, '/camera/mycamera/color/image_raw', self.color_callback, 10)
        self.depth_sub = self.create_subscription(
            Image, '/camera/mycamera/depth/image_rect_raw', self.depth_callback, 10)
        self.camera_info_sub = self.create_subscription(
            CameraInfo, '/camera/mycamera/color/camera_info', self.info_callback, 10)
    ...
    def color_callback(self, msg):
        ...
        print("[DEBUG] Received color image")
    ...
    def publish_detections(self):
        ...
        if self.color_image.dtype != np.uint8 or self.color_image.ndim != 3:
            self.get_logger().warn("Invalid image format for YOLO")
            return


        results = self.model.predict(self.color_image, imgsz=640, conf=0.7)[0]
        if len(results.boxes) == 0:
            self.get_logger().info("No blocks detected in frame")
        ...
        for idx, box in enumerate(results.boxes.xyxy.cpu().numpy()):
            ...
            try:
                # Fixed transform from camera to base_link
                offset_translation = np.array([0.0, 0.0, 0.5])  # meters
                offset_rotation = quaternion_from_euler(0, 0, 0)  # assuming no rotation

                # Construct 4x4 transform matrix manually
                transform_mat = np.eye(4)
                transform_mat[:3, 3] = offset_translation
            ...
            print(f"Block detected at x={point_base[0]:.2f}, y={point_base[1]:.2f}, z={point_base[2]:.2f}")
      ...
      f"pos=({block.position.x:.2f}, {block.position.y:.2f},             {block.position.z:.2f})")
        ...
        annotated = results.plot()  # YOLO's built-in drawing
